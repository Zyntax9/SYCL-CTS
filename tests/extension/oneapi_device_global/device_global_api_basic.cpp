...
void run_test(util::logger& log, const std::string& type_name) {
  ...
  util::array<bool, integral(indx::size)> result;
  {
    ...
    queue.submit([&](sycl::handler& cgh) {
      ...
      cgh.single_task<kernel>([=] {
        ...
        result_acc[integral(indx::is_def_value)] = (default_value == instance);
  ...
  for (size_t i = integral(indx::is_def_value); i < integral(indx::size); ++i) {
    if (!result[i]) {
      std::string fail_msg =
  ...
  util::array<bool, integral(indx::size)> result;
  {
    ...
    queue.submit([&](sycl::handler& cgh) {
      ...
      cgh.single_task<kernel>([=] {
        ...
        // Check that resulted reference is to default value
        const T& def_value{};
        result_acc[integral(indx::correct_def_val)] = (instance == def_value);
